...
package com.gestureworks.analysis 
{
	...
	public class CoreTemporalMetric
	{
		...
		public var temp_tOList
		
		private var DIM:uint; 
		
		public function CoreTemporalMetric() {
...
//			if (ts.traceDebugMode) trace("init gesture discrete analysis");
			...
			//temp_tOList = GestureGlobals.gw_public::temp_tOList;
		}
		...
		{	
			...
			//fix//--	TimelineHistories.historyQueue(tiO);		// push histories
		...
		public function findGesturePoints():void {
		
			for each (var gpt:GesturePointObject in tiO.frame.gesturePointArray){
			
				//trace("got gpt position",gpt.position);
				
				hitTestGesturePointsAddLocalGlobal(gpt,temp_tOList)
			}
		...
		public function hitTestGesturePointsAddLocalGlobal(gpt:GesturePointObject,temp_tOList:Array):void
			{
				///////////////////////////////////////////////////////////////////////////////////////////////////////////////
				// gesture point hit test
				///////////////////////////////////////////////////////////////////////////////////////////////////////////////
				
				temp_tOList = GestureGlobals.gw_public::temp_tOList;
				
				//trace("ito length",temp_tOList.length);
				
				for (var i:int = 0; i < temp_tOList.length; i++) 
				{
					var ts = temp_tOList[i];
					var temporalGesture = false;
					var gn = ts.gO.pOList.length;
					var key:uint
					
					// refactor should only happen once
					// reset once gesture list updated
					...
					if(temporalGesture){
								if ((gpt)&&(gpt.type=="tap" ||gpt.type=="touch_finger_tap")&&(gpt.mode=="touch")&&(ts.touchClusterMode == "local_strong" || ts.touchClusterMode == "local_weak"))
								{
										...
										{
											if ((ts.hitTestPoint(gpt.position.x, gpt.position.y, true))&&((!ts.mouseChildren)||(!ts.interactionPointChildren)))
											{
												var data = new Object();
												...
												tiO.frame.gestureEventArray.push(gpt_event);
												//propagateGesturePoint(gpt, ts); //TODO: develop gesture point propogation
												//trace("tap hit itouch object",ts, gpt_event, gpt_event.value.target, gpt_event.value.x)
												return;
											}
										}
										...
										{
											if (hitTest3D != null) {
												if ((hitTest3D(ts as ITouchObject3D, gpt.position.x, gpt.position.y))&&((!ts.mouseChildren)||(!ts.interactionPointChildren)))
												{ 
													var data = new Object();
														data.x = gpt.position.x;
														data.y = gpt.position.y;
													...
													tiO.frame.gestureEventArray.push(gpt_event);
													//propagateGesturePoint(gpt, ts); //TODO: develop gesture point propogation
												//	trace("tap hit itouchobject3d")
													return;
												}
											}
										}
										// ADD GLOBAL GESTURE INTERACTIONPOINTS
								...
								else if ((ts.touchClusterMode == "global") && (ts.touchEnabled) && (gpt) && (gpt.type=="tap"||gpt.type=="touch_finger_tap")&& (gpt.mode == "touch")) 
								{
									...
									tiO.frame.gestureEventArray.push(gpt_event);
									//propagateGesturePoint(gpt, ts); //TODO: develop gesture point propogation
									//trace("tap GLOBAL",ts, gpt_event, gpt_event.value.target, gpt_event.value.x)
								}
					}
					}
			}
							...
							for (var j:int = 0; j < ts.tiO.frame.pointEventArray.length; j++) 
									...
									if (tiO.frame.pointEventArray[j].type == "gwTouchEnd" || tiO.frame.pointEventArray[j].type == "touchEnd") 
									{
										...
										{	
											...
											findGestureTap(tiO.frame.pointEventArray[j], key) ; // tap event pairs
								...
								for (var k:int = 0; k < tiO.history[0].gestureEventArray.length; k++) 
									{
										if (tiO.history[0].gestureEventArray[k].type == "tap" ) {
											...
											if ((gO.pOList[key].gesture_type == "double_tap") && (!dtapCalled))	
											{
												...
												findGestureDoubleTap(tiO.history[0].gestureEventArray[k], key);
												dtapCalled = true;
											}
											...
											if ((gO.pOList[key].gesture_type == "triple_tap")&& (!ttapCalled))
											{
												...
												findGestureTripleTap(tiO.history[0].gestureEventArray[k], key);
												ttapCalled = true;
											}
		...
		// need to loop through all the touchsprites
		...
		public function countDoubleTapEvents(key:int):void // count taps each frame
		{
			...
			var ddn:uint = ts.gO.pOList[key].dList.length;
				...
				for (var i:uint = 0; i < dct; i++) // 20 fames block for single tap
					...
					{
						//trace("dtap event count", dtapEventCount)
						if((dtap_number && dtapEventCount == dtap_number) ||(!dtap_number && dtapEventCount >= dtap_number_min && dtapEventCount <= dtap_number_max))
						{
			...
			var tdn:uint = ts.gO.pOList[key].dList.length;
				...
				for (var i:uint = 0; i < tct; i++) // 20 fames block for single tap
					...
					if (ttapEventCount != 0) 
					{
						...
						if((ttap_number && ttapEventCount == ttap_number) ||(!ttap_number && ttapEventCount >= ttap_number_min && ttapEventCount <= ttap_number_max))
						{
